...
function app() {
	...
	class Car {
		constructor(color, x, z, name) {
		  ...
		  var carBodyGeometry = new THREE.TorusBufferGeometry(2, 1, 16, 32);
		  var carBodyMaterial = new THREE.MeshLambertMaterial({ color: color });
		  ...
		  var carBody_BSP = CSG.fromMesh(carBody);
		  var carBodyFinal_BSP = carBodyBase_BSP.subtract(carBody_BSP);
		  var carBodyFinal = CSG.toMesh(carBodyFinal_BSP, carBodyBase.matrix);
		  carBodyFinal.material = new THREE.MeshLambertMaterial({ color: 0x171717
		  });
		  carBodyFinal.position.set(0, 0.5, 0);
		  carBodyFinal.castShadow = true;
		  this.mesh.add(carBodyFinal);
	  
		  // Wheels
		  var wheelGeometry = new THREE.CylinderBufferGeometry(1, 1, 0.5, 24);
		  var wheelMaterial = new THREE.MeshLambertMaterial({ color: 0x171717 });
		  ...
		  wheelPositions.forEach(position => {
			...
			wheelClone.name = position.name;
			wheelClone.position.set(position.x, position.y, position.z);
			this.mesh.add(wheelClone);
		  });
	  
		  // Windows
		  var windowMaterial = new THREE.MeshLambertMaterial({ color: blur, transparent: true, opacity: 0.3 });
	  
		  var horizontalWindowGeometry = new THREE.PlaneBufferGeometry(4.4, 0.8);
		  var horizontalWindow = new THREE.Mesh(horizontalWindowGeometry, windowMaterial);
		  horizontalWindow.receiveShadow = true;
	  
		  var frontWindow = horizontalWindow.clone();
		  frontWindow.position.set(0, 1.4, -2.01);
		  frontWindow.rotation.y = Math.PI;
		  this.mesh.add(frontWindow);
	  
		  var backWindow = horizontalWindow.clone();
		  backWindow.position.set(0, 1.4, 3.01);
		  this.mesh.add(backWindow);
	  
		  var leftSideWindow = horizontalWindow.clone();
		  leftSideWindow.position.set(-2.51, 1.4, 0.4);
		  leftSideWindow.rotation.y = -0.5 * Math.PI;
		  this.mesh.add(leftSideWindow);
	  
		  var rightSideWindow = horizontalWindow.clone();
		  rightSideWindow.position.set(2.51, 1.4, 0.4);
		  rightSideWindow.rotation.y = 0.5 * Math.PI;
		  this.mesh.add(rightSideWindow);
	  
		  // Lights
		  var lightGeometry = new THREE.PlaneBufferGeometry(1, 0.5);
		  var frontLightMaterial = new THREE.MeshLambertMaterial({ color: 0xffffff });
		  var frontLight = new THREE.Mesh(lightGeometry, frontLightMaterial);
		  ...
		  var backLightMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 });
		  var backLight = new THREE.Mesh(lightGeometry, backLightMaterial);
	  ...
	  class Truck {
		constructor(color, x, z, name) {
		  ...
		  var cutFront = new THREE.Mesh(new THREE.BoxBufferGeometry(5, 1.5, 2.5));
		  var cutBack = new THREE.Mesh(new THREE.BoxBufferGeometry(5, 1.5, 4.5));
		  var frontTireHoles = new THREE.Mesh(new THREE.CylinderBufferGeometry(1.2, 1.2, 5, 16, 16, false));
		  var backTireHoles = frontTireHoles.clone();
		  var trunk = new THREE.Mesh(new THREE.BoxBufferGeometry(4.4, 1.5, 4));
		  ...
		  var cutFront_BSP = CSG.fromMesh(cutFront);
		  var cutBack_BSP = CSG.fromMesh(cutBack);
		  var frontTireHoles_BSP = CSG.fromMesh(frontTireHoles);
		  var backTireHoles_BSP = CSG.fromMesh(backTireHoles);
		  var trunk_BSP = CSG.fromMesh(trunk);
		  var truckBody_BSP = truckBodyBase_BSP
			.subtract(cutFront_BSP)
			.subtract(cutBack_BSP)
			.subtract(frontTireHoles_BSP)
			.subtract(backTireHoles_BSP)
			.subtract(trunk_BSP);
		  var truckBody = CSG.toMesh(truckBody_BSP, truckBodyBase.matrix);
		  ...
		  var horzWindowGeo = new THREE.PlaneBufferGeometry(4.4, 1.2);
		  var horzWindowMat = windowMat;
		  var horzWindow = new THREE.Mesh(horzWindowGeo, horzWindowMat);
	  
		  var midFrontWindowGeo = new THREE.PlaneBufferGeometry(1.4, 1.2);
		  var midFrontWindowMat = windowMat;
		  var midFrontWindow = new THREE.Mesh(midFrontWindowGeo, midFrontWindowMat);
	  
		  var midBackWindowGeo = new THREE.PlaneBufferGeometry(1, 1.2);
		  var midBackWindowMat = windowMat;
		  var midBackWindow = new THREE.Mesh(midBackWindowGeo, midBackWindowMat);
		  ...
		  var frontLightMat = new THREE.MeshLambertMaterial({
			color: 0xf1f1f1
		  });
		  var frontLight = new THREE.Mesh(lightGeo, frontLightMat);
		  var backLightMat = new THREE.MeshLambertMaterial({
			color: 0xf65555
		  });
		  var backLight = new THREE.Mesh(lightGeo, backLightMat);
